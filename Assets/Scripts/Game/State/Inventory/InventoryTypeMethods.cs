...
namespace Game.State.Inventory
{
    internal static class InventoryTypeMethods
    {
        ...
        public static string GetString(this InventoryType type)
        {
            //Шанс % = 100 / x
            return type switch
            {
                InventoryType.TowerCard => "Карточка башни", 
                InventoryType.SkillCard => "Карточка навыка",
                InventoryType.TowerPlan => "Чертеж башни", 
                InventoryType.SkillPlan => "Чертеж навыка",
                InventoryType.SoftCurrency => "Золото", 
                InventoryType.HardCurrency => "Кристалы",
                InventoryType.HeroCard => "Карточка героя обычна", 
                _ => throw new ArgumentOutOfRangeException(nameof(type), type, null)
            };
        }
    }
